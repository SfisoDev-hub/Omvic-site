  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-colors duration-300 ease-industrial ${
        isScrolled || isOpen ? 'bg-charcoal-900/97 shadow-panel backdrop-blur' : 'bg-transparent'
      }`}
    >
      <nav className="container-wide flex h-20 items-center justify-between" aria-label="Primary">
        <a href="#top" className="font-display text-2xl font-extrabold tracking-tight text-sand-50">
      ...
      {isOpen && (
        <div className="border-t border-charcoal-700 bg-charcoal-900 px-5 pb-8 pt-4 lg:hidden">
            ...
            {siteConfig.nav.map((item) => (
              <li key={item.href}>
                <a
                  ...
                  className="block py-3 text-base font-medium text-charcoal-100 border-b border-charcoal-700 hover:text-gold-light"
                >
                  {item.label}
                </a>
              </li>
          ...
          <div className="mt-6 flex flex-col gap-3">
            ...
            <Button href="#contact" variant="primary" onClick={handleNavClick} className="w-full">
              Request a Quote
            </Button>
          </div>
        </div>
